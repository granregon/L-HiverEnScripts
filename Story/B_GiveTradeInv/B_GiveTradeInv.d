...
func void B_GiveTradeInv(var C_Npc slf)
{
	...
	var C_Npc trd_anselm;
	var C_Npc trd_selivan;
	var C_Npc trd_romil;
	...
	trd_carl = Hlp_GetNpc(VLK_461_Carl);
	Trd_Constantino = Hlp_GetNpc(VLK_417_Constantino);
	trd_anselm = Hlp_GetNpc(vlk_4171_anselm);
	trd_selivan = Hlp_GetNpc(vlk_4172_selivan);
	trd_romil = Hlp_GetNpc(vlk_4173_romil);
	...
	trd_jan = Hlp_GetNpc(DJG_714_Jan);
	...
	if(slf.aivar[AIV_ChapterInv] <= Kapitel)
	{
		...
		if(Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(trd_carl))
		{
			B_ClearJunkTradeInv(slf);
			b_givetradeinv_carl(slf);
		};
		...
		if(Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(trd_anselm))
		{
			B_ClearJunkTradeInv(slf);
			b_givetradeinv_anselm(slf);
		};
		if(Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(trd_selivan))
		{
			B_ClearJunkTradeInv(slf);
			b_givetradeinv_selivan(slf);
		};
		if(Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(trd_romil))
		{
			B_ClearJunkTradeInv(slf);
			b_givetradeinv_romil(slf);
		};
		...
		if(Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(trd_jan))
		{
			B_ClearJunkTradeInv(slf);
			b_givetradeinv_jan(slf);
		};
