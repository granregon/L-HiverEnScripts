...
func void B_DragonKillCounter(var C_Npc current_dragon)
{
	...
	if(Hlp_GetInstanceID(current_dragon) == Hlp_GetInstanceID(amort))
	{
		if(AMORTISDEAD == FALSE)
		{
			Wld_SendTrigger("EVT_AMORT_TRAP_01");
			AMORTISDEAD = TRUE;
		};
	};
	if(Hlp_GetInstanceID(current_dragon) == Hlp_GetInstanceID(Esteban))
	{
		if((ESTEBANISDEAD == FALSE) && (ESTEBAN_FIGHTING == TRUE))
		{
			B_Attack(Fisk,Wache_01,AR_NONE,1);
			B_Attack(Fisk,Wache_02,AR_NONE,1);
			B_Attack(Wache_01,Fisk,AR_NONE,1);
			B_Attack(Wache_02,Fisk,AR_NONE,1);
			ESTEBAN_FIGHTING = FALSE;
			ESTEBANISDEAD = TRUE;
		};
	};
