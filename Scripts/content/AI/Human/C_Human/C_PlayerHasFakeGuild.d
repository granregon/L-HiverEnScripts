...
func int C_PlayerHasFakeGuild(var C_Npc slf,var C_Npc oth)
{
	...
	else if(slf.guild == oth.guild)
	{
		...
		else if((oth.guild == GIL_KDF) && ((Hlp_IsItem(itm,ItAr_KDF_L) == TRUE) || (Hlp_IsItem(itm,itar_kdf_l_wh) == TRUE) || (Hlp_IsItem(itm,ItAr_KDF_H) == TRUE) || (Hlp_IsItem(itm,itar_kdf_h_wh) == TRUE) || (Hlp_IsItem(itm,itar_kdw_h) == TRUE) || (Hlp_IsItem(itm,itar_kdw_h_wh) == TRUE)))
		{
			return FALSE;
		}
		else
