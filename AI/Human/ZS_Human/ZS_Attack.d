...
func int ZS_Attack_Loop()
{
	...
	if(hero.aivar[91] == TRUE)
	{
		if(C_BodyStateContains(hero,BS_PARADE) && (Npc_GetDistToNpc(self,hero) > 550) && Npc_IsInFightMode(self,FMODE_FAR))
		{
			b_sh_blockchance();
			b_sh_blocksnd();
		};
	...
	{
		if(!c_schasitemshield(self) && (self.aivar[92] != SH_FALSE))
		{
			self.aivar[92] = SH_FALSE;
			B_AddFightSkill(self,NPC_TALENT_1H,0);
		};
	};
	if(Hlp_GetInstanceID(self) == Hlp_GetInstanceID(Juan))
	{
		if((Hlp_GetInstanceID(other) == Hlp_GetInstanceID(hero)) && !Npc_IsInState(Freund,ZS_Attack))
		{
			B_Attack(Freund,hero,AR_NONE,1);
		};
	};
	...
	if((LOAD_CHECK > 0) && Npc_IsInFightMode(self,FMODE_FAR))
	{
		Npc_ClearAIQueue(self);
...
func void ZS_Attack_End()
{
	...
	if(C_NpcIsDown(other) && C_WantToRansack(self) && ((other.aivar[AIV_RANSACKED] == FALSE) || C_NpcRansacksAlways(self)) && (Npc_GetDistToNpc(self,other) < PERC_DIST_INTERMEDIAT))
	{
		...
		if((other.guild < GIL_SEPERATOR_HUM) && (Npc_GetDistToWP(self,"NW_CASTLEMINE_HUT_01") > 2500))
		{
