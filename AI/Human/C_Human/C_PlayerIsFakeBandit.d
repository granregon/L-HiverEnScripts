...
func int C_PlayerIsFakeBandit(var C_Npc slf,var C_Npc oth)
{
	...
	if(Npc_HasEquippedArmor(oth) == TRUE)
	{
		...
		if((Hlp_IsItem(itm,ItAr_BDT_M) == TRUE) || (Hlp_IsItem(itm,itar_oldcamp_guard_l) == TRUE) || (Hlp_IsItem(itm,itar_oldcamp_guard_m) == TRUE) || (Hlp_IsItem(itm,itar_oldcamp_guard_h) == TRUE) || (Hlp_IsItem(itm,ItAr_BDT_H) == TRUE))
		{
			return TRUE;
		}
		else
